  ...
  if(yes) then
     ...
     do m=1,mblocks
        do L = 1,ions(mn)%block(m)%mi
            !L大概是这个block里面离子的编号
            !这个block里面一共有ions(mn)%block(m)%mi个离子？？？？？？？？？？？？
           ...
           do n = 1,nregion
			  if(p>= domain_pregion(n,1) .and. p<= domain_pregion(n,2) .and. & 
			     ...
			     vpar = ions(mn)%block(m)%mark(1,L)
              vper1 = ions(mn)%block(m)%mark(2,L)
              vper2 = 0;

            !下面几行的意思：如果send数组不够存要输出的离子，把send数组扩容到原来的1.5倍
				 if(num >=  nbuff) then
            ...
            !mn：粒子种类，整数
            !p,q,w：位置坐标，单位是格点数
            !vx,vy,vz：三个速度。所有速度的单位都是模拟中速度的单位
            !vpar：平行速度，平行于磁场方向的速度
            !vper1、vper2：垂直方向速度，汪老师知道垂直方向怎么选的
            !frac：粒子的权重，汪老师知道 ions(mn)%block%...取默认值1
				 goto 110
             !goto 110：如果离子在其中一个观测区间就直接输出了。已经输出就不用再去判断它是否在别的观测区间，
             !          直接开始判断下一个离子要不要输出。这样省去很多判断，并且一个离子不会被重复输出。
		       endif  
...
!    ------------------------- reduce number of particles here ---------------
     ...
     !一个thread要输出num个离子，各个thread的num不同
     !把所有thread的num加起来，结果放在numt里面。所有thread都有一份numt
	 num0     = num
	 num_skip = max(numt/num_diag,1)!设置这个参数的意义见下面的do循环
	 num      = 0
	 do m=1,num0,num_skip!循环从1开始，到m>num0截止，m每次增加num_skip
    !num_diag是所有线程输出的最大粒子数。
    !如果num_diag = 1000，但是一共有numt=2000个粒子要输出
    !那么num_skip=2，这个do循环就会输出1，3，5，....，1999号共1000个粒子
        num = num+1
		send(:,num) = send(:,m)
	 enddo
!    --------------------------------------------------------
     call mpi_allgather(num,1,mpi_integer,num_recv,1,mpi_integer,MPI_COMM_WORLD,ier)
     !从num的地址发送1个类型为mpi_integer的数据
     !接收的数据为1个mpi_integer，放在num_recv对应的地址
     !MPI_COMM_WORLD：所有线程都要接收

     if(mype > 0 .and. num > 0) then
         !不是master thread，并且有要输出的离子
	       call mpi_isend(send(:,1:num),ind*num, mpi_real,0, &
		                           mype, MPI_COMM_WORLD, request(mype),ier)
         !发送：从本线程send数组的头指针处开始发送，发送ind*num个mpi_real（其实就是把send(:,1:num)发出去）
         !接收：负责接收的是master thread，消息标签为发送线程的编号，通信域MPI_COMM_WORLD
         !request(mype)：不知道在干啥？？？？？？？？？？？？？？？？？
     endif

     !master thread中要输出的离子数据已经抄写到send数组中
     !下面这个if的作用：让master thread把send数组抄写到QVP_writi里面
     !master thread如果发现QVP_writi空间不够存了就把QVP_writi扩容1.5倍
     if(mype==0) then
	    Li_tot = 0
        do i=1,num
		   Li_tot = Li_tot +1
     ...
     do L=1,numberpe-1!遍历所有的子线程，L为线程编号
	    if(num_recv(L+1) > 0) then!如果这个子线程输出离子数据数量不为0
	       if(mype==0) then!master thread才进行接收操作
		      allocate(recv(ind,num_recv(L+1)))
              call mpi_recv(recv,ind*num_recv(L+1), mpi_real,L, &
		                           L, MPI_COMM_WORLD, status, ier)
               !从MPI_COMM_WORLD中的L号线程接收ind*num_recv(L+1)个离子的数据。
               !放在master thread的recv数组中
		  
            !下面的do的大致作用：如果master thread的数组不够大，
            !不够接收数据，则把QVP_writi扩容到原来的1.5倍
		      do i=1,num_recv(L+1)
		         Li_tot = Li_tot +1
		         if(Li_tot >= nbuffi) then
                    allocate(dummy(ind,Li_tot-1))
			  ...
			  deallocate(recv)
           elseif(mype==L)then
           !如果不是master_thread，等自己线程发送的数据被master_thread接收之后再进行下一步操作？？？？？？？？
           !不然还没发送完，离子又开始运动，乱套了
   ...
   !现在所有离子的数据都倍接收到master thread的QVP_writi数组里面了
        ...
        do L = 1,eles(mn)%block(m)%mi
           ...
           do n = 1,nregion
			  if(p>= domain_pregion(n,1) .and. p<= domain_pregion(n,2) .and. & 
			     ...
			     vpar = eles(mn)%block(m)%mark(1,L)
              vper1 = eles(mn)%block(m)%mark(2,L)
              vper2 = 0;
...
! ------------------------------
  ...
  if(Li_tot+Le_tot == 0 ) goto 831 !如果一个粒子都没有输出，那么......  
831  continue
   ...
   if( mype == 0) then
      ...
      write(33,*) 125
      Write(33,1106) param(1:125)
      write(33,1106) QVP_writi(:,1:Li_tot)
      write(33,*) 125125125 ! use loadparticle.m if this line is commented, otherwise use loadp_f.m
      write(33,1106) QVP_write(:,1:Le_tot)
      close(33)
   endif
