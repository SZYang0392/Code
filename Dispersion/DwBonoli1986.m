function varargout = DwBonoli1986(w, k_para, k_perp, B, Ps, spec)
...
% COLLISION EFFECT IS NOT INCLUDED
    ...
    DwDre = 0;
    DwDre = DwDre - (6*P6.*N_perp2.^3 + 4*P4.*N_perp2.^2 + 2*P2.*N_perp2)./w;
    P6w = (3*P6I - 0.75*P6E)./w;
    P4w = (2./w).*WPI;
    P2w = (2./w).*((2*WPI + WPE).*(N_para2 - e_perp) - (e_para + e_perp).*(N_para2 + WPI) - e_xy.^2);
    P0w = (2./w).*((1 - e_para).*((N_para2 - e_perp).^2 - e_xy.^2) +...
        e_para.*(- 2*(N_para2 - e_perp).*(N_para2 + WPI) + e_xy.^2));
    DwDre = DwDre + P6w.*N_perp2.^3 + P4w.*N_perp2.^2 + P2w.*N_perp2 + P0w;
    varargout{1} = D;
    varargout{2} = DwDre;
    varargout{3} = 1i*DsIm;
end
